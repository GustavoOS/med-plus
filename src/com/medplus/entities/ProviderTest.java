...
class ProviderTest {
	...
	void testGettersAndSetters() {
		hp = new Provider();
		hp.setLocal((new CoordinateDS()).with(-23.1735709, -45.8423158));
		...
		assertEquals(-23.173, hp.getLocal().lat(), 0.01);
		assertEquals(-45.842, hp.getLocal().lon(), 0.01);
	...
	void testParameterizedConstructor() {
		hp = new Provider(
				...
				"surgeon",
				(new CoordinateDS()).with(-23.1735709, -45.8423158));
		assertEquals("João", hp.getName());
		assertEquals(-23.173, hp.getLocal().lat(), 0.01);
		assertEquals(-45.842, hp.getLocal().lon(), 0.01);
		assertEquals("https://www.instagram.com/neymarjr/", hp.getSocialMediaURL());
